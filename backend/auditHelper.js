...
const auditHelper = {
    ...
    logAction: db.transaction((caseId, actorId, action) => {
        ...
        // Compute entry_hash = SHA-256(prev_hash + actor_id + action + timestamp + case_id)
        const payload = prev_hash + actorId + action + timestamp + caseId;
        const entry_hash = crypto.createHash('sha256').update(payload).digest('hex');
        
        const insertStmt = db.prepare(`
            INSERT INTO audit_log (case_id, actor_id, action, timestamp, prev_hash, entry_hash) 
            VALUES (?, ?, ?, ?, ?, ?)
        `);
        insertStmt.run(caseId, actorId, action, timestamp, prev_hash, entry_hash);
